...
export default class RyanScreenshotFixes extends Common {
    ...
    setInterval(() => {
        try {
            ...
            parentElements.forEach(parent => {
                const carouselElements = parent.querySelectorAll('.product-gallery__carousel.scroll-area');
  ...
  private SANTOFIremoveThumbnailStyles() {
    setInterval(() => {
        try {
            const thumbnailImages = this.dom.querySelectorAll('.summary-thumbnail.img-wrapper > img');
  ...
  private FriskChocolatessetVideoWidth() {
    setInterval(() => {
        try {
            const videoElements = this.dom.querySelectorAll('.reelUp_card_video > video');
            if (!videoElements.length) return;
            
            videoElements.forEach(video => {
                (video as HTMLElement).style.setProperty('width', '-webkit-fill-available');
            });
        } catch (error) {
            return;
        }
  ...
  private VIIAhideIframe() {
    setInterval(() => {
        try {
            const iframe = this.dom.querySelector('iframe#_jst-iframe-75a4af7fddbe4af48543a23cf3b93b90-11b581a07cfe11ef9fe53b8b0b0ca0d7-41e20832106b4c139ce932ba994c3702');
            if (!iframe) return;
