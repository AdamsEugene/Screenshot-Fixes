...
class ScreenshotFixes extends Common {
  ...
  private RetroSnapsetHeaderHeight = () => {
    setInterval(() => {
      try {
        const headerSection = this.dom.querySelector(
        ...
        if (actualHeight) {
          (headerSection as HTMLElement).style.setProperty(
            "max-height",
            actualHeight
          );
        }
  ...
  private createFunctionsMap(): Record<number, (() => void)[]> {
    const functionGroups: {
      ...
      { ids: [3024], functions: [this.BentgoupdateHeaderPadding] },
      {
        ids: [3065],
        functions: [
          this.blocoutsetSubmenuHeight,
          this.BlocoutremoveImgHeight,
          this.BlocoutsetSlickTrackStyles,
        ],
      },
